...
const setData = async (data: string) => {
  await AsyncStorage.setItem('AUTH_PHONE_BLOCK', data);
};
...
function TimerBlockPhoneAuth({
  expiredTimer,
  isConfirm,
  callBack,
}: TimerBlockPhoneProps) {
  const { isRecoveryByPhone, isActivePhoneTimer } = useAppSelector(selectAuth);
  ...
  const handleBlock = useCallback(() => {
    ...
    setData(JSON.stringify(data)).then(() => {
      setIsBlock(data);
      dispatch(timerOnAuthPhone());
      dispatch(setIsRecoveryByPhone(false));
    });
  }, []);

  const closeBlock = useCallback(() => {
    AsyncStorage.removeItem('AUTH_PHONE_BLOCK').then(() => {
      setIsBlock({ block: false, timerOffset: null });
      dispatch(timerOffAuthPhone());
    });
  }, [isBlock]);
  ...
  useEffect(() => {
    const readStorage = async () => {
      try {
        const value = await AsyncStorage.getItem('AUTH_PHONE_BLOCK');
        if (value === null) {
          dispatch(timerOffAuthPhone());
        }
        if (value !== null) {
...
export default TimerBlockPhoneAuth;
